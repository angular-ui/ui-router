...
function ng1ViewsBuilder(state) {
    ...
    if (ui_router_core_1.isDefined(state.views) && hasAnyKey(allViewKeys, state)) {
        throw new Error("State '" + state.name + "' has a 'views' object. " +
            "It cannot also have \"view properties\" at the state level.  " +
            "Move the following properties into a view (in the 'views' object): " +
            (" " + allViewKeys.filter(function (key) { return ui_router_core_1.isDefined(state[key]); }).join(", ")));
    }
    var views = {}, viewsObject = state.views || { "$default": ui_router_core_1.pick(state, allViewKeys) };
    ui_router_core_1.forEach(viewsObject, function (config, name) {
        ...
        config = ui_router_core_1.extend({}, config);
