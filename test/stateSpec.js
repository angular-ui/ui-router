describe('state', function () {
  ...
  describe('.get()', function () {
    ...
    it('should work for relative states', inject(function ($state) {
      ...
      var person = $state.get('.person', { relative: about });
      expect(person.url).toBe('/:person');
      expect($state.get('^', { relative: 'about.person' }).url).toBe('/about');

      var item = $state.get('.person.item', { relative: about });
      expect(item.url).toBe('/:id');
      expect($state.get('^.^', { relative: item }).url).toBe('/about');
    }));
  });
