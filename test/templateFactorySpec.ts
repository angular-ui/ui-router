...
describe('templateFactory', function() {
  ...
  if (angular.version.minor >= 3) {
    ...
    describe('should follow $sce policy and', function() {
      ...
      it(
        'accepts explicitly trusted URLs',
        inject(function($templateFactory, $httpBackend, $sce) {
  ...
  if (angular.version.minor <= 2) {
    ...
    it(
      'does not restrict URL loading',
      inject(function($templateFactory, $httpBackend) {
        $httpBackend.expectGET('http://evil.com/views/view.html').respond(200, 'template!');
        $templateFactory.fromUrl('http://evil.com/views/view.html');
        $httpBackend.flush();
    ...
    it(
      'should request templates as text/html',
      inject(function($templateFactory, $httpBackend) {
        $httpBackend
  ...
  if (angular.version.minor >= 5) {
    describe('component template builder', () => {
      ...
      beforeEach(
        inject(($uiRouter, $compile, $rootScope) => {
          router = $uiRouter;
          rootScope = $rootScope;
